...
Authorization.validateSignature = function(client, message){
  ...
  // Always passes if we're not setup for subscription signatures
  if (typeof Config.subscriptionSharedSecret === 'undefined') {
    return true;
  }
  ...
  hash.update([Config.subscriptionSharedSecret, channelName, timestamp].join(':'));
